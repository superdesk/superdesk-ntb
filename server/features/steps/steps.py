...
@when(
    'we fetch events "{mock_requests}" from "{provider_name}" NTB Events API provider'
)
def step_impl_when_fetch_from_ntb_events_api_ingest(
    context, mock_requests, provider_name
):
    """
    ...
    with responses.RequestsMock() as rsps:
        ...
        for filename in mock_requests.split(","):
            file_path = os.path.join(
                provider.get("config", {}).get("fixtures_path"), filename
            )
            with open(file_path, "rb") as f:
                rsps.get(provider["config"]["url"], body=f.read(), status=200)
...
def step_impl_fetch_from_provider_ingest(context, provider_name, guid):
    with context.app.test_request_context(context.app.config["URL_PREFIX"]):
        ...
        ingest_provider_service.system_update(
            provider["_id"], {LAST_ITEM_UPDATE: utcnow()}, provider
        )

        for item in items:
            set_placeholder(
                context, "{}.{}".format(provider_name, item["guid"]), item["_id"]
            )
...
def step_imp_when_action_resource(context, action, resource, item_id):
    ...
    context.response = context.client.patch(
        get_prefixed_url(context.app, action_url),

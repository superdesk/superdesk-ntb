...
def mock_contacts_find_one(return_value):
    superdesk.resources["contacts"].service.find_one = MagicMock(
...
class NTBEventTestCase(TestCase):
            ...
            "location": [
                {
                    ...
                    "address": {
                        ...
                        "locality": "Prague",
                        "postal_code": "11000",
                        "line": ["1092/10 Salvatorska street"],
                    },
                },
            ],
            "links": [
    ...
    def test_formatter(self):
        ...
        geo = root.find("geo")
        self.assertEqual(
            str(self.item["location"][0]["location"]["lat"]), geo.find("latitude").text
        )
        self.assertEqual(
            str(self.item["location"][0]["location"]["lon"]), geo.find("longitude").text
        )
    ...
    def test_location(self):
        ...
        root = lxml.etree.fromstring(output["encoded_item"])
        location = root.find("location")
        self.assertEqual(location.text, "Prague, Old Town, Prague, 11000, Czechia")
    ...
    @mock.patch(
        "apps.contacts.service.ContactsService.find_one",
        return_value={"contact_email": ["john.smith@nasa.org"]},
    )
    @mock.patch.dict("superdesk.resources", resources)
    ...
    def test_contactmail_not_included(self):
        mock_contacts_find_one({"contact_email": []})
    ...
    def test_contactphone_included(self):
        mock_contacts_find_one({"contact_phone": [{"number": "99999", "public": True}]})

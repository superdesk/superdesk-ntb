...
class BaseNTBTTNINJSTestCase(TestCase):
    ...
    def setUp(self):
        ...
        with open(
            os.path.normpath(
                os.path.join(dirname, "../fixtures", self.sanitisedhtmlfilename)
            ),
            "r",
        ) as f:
...
class NTBTTNINJSTestCase(BaseNTBTTNINJSTestCase):
    ...
    def test_item(self):
        ...
        expected_subject = {"name": "Sport", "scheme": "category", "qcode": "Sport"}
        self.assertIn(expected_subject, item["subject"])
        expected_anpa_category = {"name": "Nyhetstjenesten", "qcode": "n"}

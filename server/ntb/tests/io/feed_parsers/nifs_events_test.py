...
VOC = [
    ...
    },
    {
        ...
        "type": "manageable",
        "unique_field": "qcode",
        "items": [{"is_active": True, "name": "Sport", "qcode": "sport"}],
    },
]
...
class NifsTestCase(BaseNIFSTestCase):
    ...
    def test_item(self):
        ...
        self.assertEqual(
            item["name"], "Fotball Eliteserien menn, 1. runde, Bodø/Glimt - Lillestrøm"
        )
        self.assertEqual(item["slugline"], "Fotball")
        self.assertEqual(
            item["dates"]["start"].isoformat(), "2018-03-11T18:00:00+01:00"
        )
        self.assertEqual(item["dates"]["end"].isoformat(), "2018-03-11T20:00:00+01:00")
        self.assertEqual(item["dates"]["tz"], "")
        self.assertEqual(
            item["anpa_category"][0], {"qcode": "n", "name": "Nyhetstjenesten"}
        )
        self.assertEqual(item["calendars"][0]["qcode"], "sport")
        self.assertIn(
            {"name": "Sport", "qcode": "Sport", "scheme": "category"}, item["subject"]
        )
        self.assertIn(
            {"qcode": "15000000", "name": "Sport", "scheme": "subject_custom"},
            item["subject"],
        )
        self.assertIn(
            {"qcode": "15054000", "name": "Fotball", "scheme": "subject_custom"},
            item["subject"],
        )

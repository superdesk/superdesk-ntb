...
from ntb.io.feed_parsers.stt_newsml import NTBSTTNewsMLFeedParser
from . import XMLParserTestCase


class BaseSTTNewsMLTestCase(XMLParserTestCase):
    def setUp(self):
        self.parser = NTBSTTNewsMLFeedParser()
        super().setUp()
...
class STTTestCase(BaseSTTNewsMLTestCase):
    ...
    def test_can_parse(self):
        self.assertTrue(NTBSTTNewsMLFeedParser().can_parse(self.xml_root))
    ...
    def test_content(self):
        ...
        self.assertEqual(
            item["headline"],
            "Parliament passed the Alcohol Act and the government gained confidence"
            "*** TRANSLATED ***",
        )
...
class STTSportTestCase(BaseSTTNewsMLTestCase):
    ...
    def test_content(self):
        item = self.item[0]
        self.assertIn(
            {"qcode": "Sport", "name": "Sport", "scheme": "category"}, item["subject"]
        )
        self.assertIn({"name": "Nyhetstjenesten", "qcode": "n"}, item["anpa_category"])
        self.assertEqual(
            item["headline"],
            "Hämeen Sanomat: Ilmarinen hakee jääkiekkoseura Espoo Unitedia konkurssiin",
        )
        ...
        expected_body = (
            "<p>Eläkevakuutusyhtiö Ilmarinen hakee jääkiekkoseura Espoo Unitedia konkurssiin, kertoo "
            ...
            " miljoonaa euroa tappiollinen. Tilikauden liikevaihto oli 699\u2009 000 euroa.</p>\n\n<p>Esp"
            "oo United on perustettu vuonna 2016. </p>"
        )

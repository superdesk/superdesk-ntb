...
class RitzauTestCase(BaseRitzauTestCase):
    ...
    def test_content(self):
        item = self.item
        subject_expected = [
            {"qcode": "Utenriks", "name": "Utenriks", "scheme": "category"}
        ]
        self.assertEqual(item["subject"], subject_expected)
        self.assertEqual(
            item["anpa_category"], [{"name": "Nyhetstjenesten", "qcode": "n"}]
        )
        self.assertEqual(item["version"], 1)
        self.assertEqual(item["byline"], "/ritzau/")
        self.assertEqual(item["guid"], "9a6955fc-11da-46b6-9903-439ebb288f2d")
        self.assertEqual(
            item["firstcreated"].isoformat(), "2018-01-30T16:32:18.397000+00:00"
        )
        expected_body = (
            '<p>Hollandske forskere har "i årevis" lavet forsøg på både mennesker og dyr, hvor de har tes'
...
class RitzauTestCase2(BaseRitzauTestCase):
    ...
    def test_content(self):
        item = self.item
        subject_expected = [
            {"name": "Politikk", "qcode": "11000000", "scheme": "subject_custom"},
            {"name": "Forsvar", "qcode": "11001000", "scheme": "subject_custom"},
            {
        ...
        self.assertEqual(
            sorted(item["subject"], key=lambda i: i["qcode"]), subject_expected
        )
        self.assertEqual(
            item["anpa_category"], [{"name": "Nyhetstjenesten", "qcode": "n"}]
        )
        ...
        self.assertEqual(item["byline"], "/ritzau/")
        self.assertEqual(
            item["headline"], "Forsvaret sender satellit ud i rummet for første gang"
        )
        self.assertEqual(
            item["abstract"],
            "En ny, danskudviklet satellit skal udføre overvågningsopgaver i "
            "Arktis for det danske Forsvar.<br />",
        )
        self.assertEqual(item["guid"], "b5342fc2-b3e1-4331-ab2b-5e93e0d59427")
        self.assertEqual(
            item["firstcreated"].isoformat(), "2018-01-31T17:45:36.730000+00:00"
        )
        expected_body = (
            "<p>En danskudviklet minisatellit – en såkaldt cubesat - bliver sendt ud i rummet fredag, hvi"

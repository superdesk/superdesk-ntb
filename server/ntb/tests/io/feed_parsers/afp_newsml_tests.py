...
class NTBAFPNewsMLTestCase(XMLParserTestCase):
    ...
    def test_parsed(self):
        ...
        self.assertIn(
            {"name": "Nyhetstjenesten", "qcode": "n"}, self.item["anpa_category"]
        )
        self.assertEqual(self.item["urgency"], 5)
        self.assertEqual(
            self.item.get("headline"), "Burkina Faso bans imports from North Korea"
        )
...
class NoHeadlineTestCase(XMLParserTestCase):
    ...
    def test_no_headline(self):
        self.assertEqual(
            ...
            "Burkina Faso is banning imports from North Korea to comply with to"

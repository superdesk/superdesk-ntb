...
class PingScanpixTestCase(TestCase):
    ...
    def setUp(self):
        with open(
            os.path.join(os.path.dirname(__file__), "fixtures", "published_item.json")
        ) as f:
        ...
        self.app.data.insert(
            ...
            [
                {
                    "_id": bson.ObjectId(
                        self.item["associations"]["featuremedia"]["ingest_provider"]
                    ),
    ...
    def test_ping_scanpix_on_item_publish(self):
        ...
        responses.add(
            responses.GET,
            SCANPIX_DOWNLOAD_URL.format(
                "editorial", self.item["associations"]["featuremedia"]["_id"]
            ),
            body=b"",
            status=200,
        )

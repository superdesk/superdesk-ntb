    ...
    try:
        ...
        if not media:
            ...
            provider = superdesk.get_resource_service("search_providers").find_one(
                req=None, _id=item["ingest_provider"]
            )
            res = http.get(
                url,
...
def ping_scanpix(assoc, item):
    ...
    try:
        res = http.post(
            ...
            auth=(
                app.config["SCANPIX_PING_USERNAME"],
                app.config["SCANPIX_PING_PASSWORD"],
            ),
            timeout=PING_TIMEOUT,
        )
        logger.info(
            "scanpix image published status=%d image=%s article=%s",
            res.status_code,
            assoc.get("guid", ""),
            item.get("guid", ""),
        )
...
def init_app(app):
    ...
    if app.config.get("SCANPIX_PING_OWNER") and app.config.get("SCANPIX_PING_USERNAME"):
        logger.info(
            "SCANPIX ping owner configured %s", app.config["SCANPIX_PING_OWNER"]
        )
    else:

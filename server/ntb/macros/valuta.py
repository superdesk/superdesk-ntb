...
template = (
    "<tr><td>{name}&nbsp;({currency})</td><td>{today}</td><td>({yesterday})</td></tr>"
)
...
def generate_row(
    ...
    if today_currency and multiplication:
        today_rate = "{0:.4f}".format(
            float(euro_currency) * multiplication / float(today_currency)
        )

    if yesterday_currency and multiplication:
        yesterday_rate = "{0:.4f}".format(
            float(yesterday_euro) * multiplication / float(yesterday_currency)
        )
...
def get_currency(today_date):
    try:
        ...
        xpath_euro_string = (
            ".//eurofxref:Cube[@time='{date}']/eurofxref:Cube[@currency='NOK']"
        )

        euro_query = doc.find(
            xpath_euro_string.format(date=today_date.date()), namespaces
        )

        if euro_query is None:
            euro_query = doc.find(
                xpath_euro_string.format(date=yesterday_date.date()), namespaces
            )
        ...
        nodes_today = doc.findall(
            all_currencies.format(date=today_date.date()), namespaces
        )

        today_dictionary = {
            cube.attrib["currency"]: cube.attrib["rate"] for cube in nodes_today
        }

        nodes_yesterday = doc.findall(
            all_currencies.format(date=yesterday_date.date()), namespaces
        )

        yesterday_dictionary = {
            cube.attrib["currency"]: cube.attrib["rate"] for cube in nodes_yesterday
        }
...
def ntb_currency_macro(item, **kwargs):
    ...
    headline = headline.format(
        today_date.strftime("%d.%m"), yesterday_date.strftime("%d.%m")
    )
    ...
    item["body_html"] = (
        "<table>" + "".join(line for line in get_currency(today_date)) + "</table>"
    )

    generate_fields(
        item, ["headline", "abstract", "body_html"], force=True, reload=True
    )

...
def generate_feed(items):
    ...
    for item in items:
        if (
            not item.get("headline")
            and not item.get("slugline")
    ...
    return fg.atom_str(pretty=True).replace(
        b'<content type="CDATA">', b'<content type="html">'
    )
...
def get_content(item):
    ...
    if tree is None:
        logger.warning(
            "no content for item %s headline=%s body=%s",
            item["_id"],
            item.get("headline"),
            item.get("body_html"),
        )
    ...
    for elem in body:
        if elem.tag is etree.Comment and elem.text:
            m = re.fullmatch(
                r'EMBED START Image {id: "([-_a-z0-9]+)"}', elem.text.strip()
            )
            if m:
                _id = m.group(1)

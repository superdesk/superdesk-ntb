...
class TTNewsMLFeedParser(NewsMLTwoFeedParser):
    ...
    def is_sport(self, root):
        return any(
            (
                bool(
                    root.xpath(
                        '//iptc:service[@qcode="tt_prodid:%s"]' % code, namespaces=NS
                    )
                )
                for code in self.SPORT_CODES
            )
        )
    ...
    def parse(self, xml, provider=None):
        ...
        try:
            ...
            item["firstcreated"] = date_parser.parse(
                self.root.xpath("//iptc:contentCreated/text()", namespaces=NS)[0]
            )
    ...
    def parse_inline_content(self, tree, item):
        ...
        etree.strip_tags(
            body_elt, "TEXT", "FAKTATEXT", "BAKGRUNDTEXT", "INGRESS", "BRODTEXT"
        )
        ...
        div_elt[:] = body_elt[:]
        contents = [
            etree.tostring(e, encoding="unicode", method="html") for e in div_elt
        ]
        return {"content": "\n".join(contents)}

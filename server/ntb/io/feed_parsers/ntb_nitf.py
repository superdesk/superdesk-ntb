...
def build_subject(xml):
    voc_subjects_map = superdesk.get_resource_service("vocabularies").find_one(
    ...
    for elem in xml.findall("head/tobject/tobject.subject"):
        ...
        name = elem.get(
            SUBJECT_TYPE, elem.get(SUBJECT_MATTER, elem.get(SUBJECT_DETAIL))
        )
        if name is None:
        ...
        try:
            voc_subject = next(
                (s for s in voc_subjects_map["items"] if s["qcode"] == qcode)
            )
...
def build_service(elem):
    ...
    voc_categories = superdesk.get_resource_service("vocabularies").find_one(
    ...
    for voc_category in voc_categories:
        if category == voc_category["name"]:
            try:
                update = {
                    "qcode": voc_category["qcode"],
                    "language": voc_category["language"],
                }
            except KeyError:
                continue
            else:
...
NITF_MAPPING = {
    "anpa_category": {
        "xpath": "head/meta[@name='NTBTjeneste']",
        "filter": build_service,
    },
    "priority": {"update": True, "xpath": "head/meta[@name='NTBPrioritet']"},
    "ntb_category": {
        "xpath": "head/tobject[@tobject.type]",
        "filter": lambda e: [
            {
                "qcode": e.get("tobject.type"),
    ...
    "genre": {
        "xpath": "head/tobject[@tobject.type]/tobject.property[@tobject.property.type]",
        "filter": lambda e: [
            {
                "qcode": e.get("tobject.property.type"),

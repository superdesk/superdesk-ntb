    ...
    def authorized(self, allowed_roles, resource, method):
        ...
        for provider in superdesk.get_resource_service("ingest_providers").get(
            ...
            if config["username"] == username and config["password"] == password:
                ...
                checksum = md5(
                    json.dumps(request.json).encode("utf-8") + secret
                ).hexdigest()
                ...
                if sys.version_info[:2] <= (3, 5):
                    ...
                    logger.warning(
                        "Checksum validation disabled on this Python version, please upgrade to Python 3.6+"
                    )
                ...
                if checksum != request.args["checksum"]:
                    ...
                    NewsworthyWebhookService.requests_map.setdefault(
                        request, []
                    ).append(provider)
                    found_provider = True
...
class NewsworthyFeedingService(FeedingService):
    ...
    fields = [
        {
            "id": "url",
            "type": "text",
            "label": "Use this URL for webhook",
            "default_value": "",
            "readonly": True,
        },
    ...
    def _update(self, provider, update):
        ...
        if data["hook"]["event"] == EVENT_UNPUBLISHED:
            logger.info(
                "ignoring unpublish event on following data:\n{data}".format(data=data)
            )
            return [[]]

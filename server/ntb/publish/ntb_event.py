...
class NTBEventFormatter(Formatter):
    ...
    def format(self, item, subscriber, codes=None):
        ...
        xml = etree.tostring(
            doc, pretty_print=True, xml_declaration=True, encoding=self.ENCODING
        )
        return [
            {
    ...
    def _format_doc(self, doc, item):
        ...
        if (
            item.get("pubstatus") == POST_STATE.CANCELLED
            or item.get("state") in DELETE_STATES
        ):
    ...
    def _format_location(self, doc, item):
        ...
        if item.get("location"):
            ...
            location.text = ", ".join(
                [chunk for chunk in location_chunks if chunk]
            ).replace("\n", " ")
    ...
    def _format_id(self, item):
        ...
        if item.get("reschedule_from"):
            parent_event = get_resource_service("events").find_one(
                req=None, _id=item.get("reschedule_from")
            )
            if parent_event:
    ...
    def _format_alldayevent(self, doc, dates):
        ...
        _pattern = "%H:%M"
        is_all_day_event = (
            local_start_time.strftime(_pattern) == "00:00"
            and local_end_time.strftime(_pattern) == "23:59"
        )
    ...
    def _format_contact_info(self, doc, item):
        ...
        if len(item.get("event_contact_info", [])) > 0:
            ...
            contact_details = get_resource_service("contacts").find_one(
                req=None, _id={"$in": item["event_contact_info"]}, public=True
            )
            ...
            if contact_details:
                ...
                name = (
                    name + organisation
                    if name == "" or organisation == ""
                    else name + ", " + organisation
                )
                ...
                phones = [
                    p
                    for p in contact_details.get("contact_phone", [])
                    if p.get("public")
                ]
                if len(phones) > 0:
